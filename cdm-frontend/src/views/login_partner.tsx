...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!employeeId.trim()) {
      setEmployeeIdError("아이디를 입력해 주세요.");
      hasError = true;
    }
    if (!password.trim()) {
    ...
    try {
      // Node(JWT) 일반사용자(CA) 로그인 API 호출
      const response = await jwtAxios.post(`/api/ca/auth/login`, {
        prgrmId: "kids-user",
        mbrId: employeeId,
      ...
      navigate(ROUTES.CM.MB.PARTNER_DASHBOARD);
    } catch {
      setEmployeeIdError("아이디/비밀번호를 확인해 주세요.");
    }
  };
        ...
        <div className="desc">
          <h1 className="logo">
            <a href="javascript:void(0);">
              ...
              <span className="logo_text">로그인</span>
            </a>
          </h1>
