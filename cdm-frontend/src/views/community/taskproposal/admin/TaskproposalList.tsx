...
export default function AdminTaskproposalListView() {
  ...
  const { optionsWithAll: categoryOptions, codeMap: statusCodeMap } = useCommonCodes("CMCMM00004");
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      fetchAsmtPrpList({
        ...
        asmtPrpAnsSttsCd: appliedState.status === "ALL" ? undefined : appliedState.status,
        searchAll: true,
      }),
    refetchOnMount: "always",
  });

  const rows: AsmtPrpItem[] = data?.list ?? [];
