...
export default function ResearchAdminView() {
  ...
  /* ------------------------------
   * 검색 핸들러 → 폼 값을 URL에 반영 (검색 버튼 클릭 시에만 필터 적용)
   * ------------------------------ */
  const handleSearch = () => {
  ...
  /* ------------------------------
   * 페이지 변경 (CdmPagination / CdmPaginationMove) → 적용된 조건 유지, page만 변경
   * ------------------------------ */
          ...
          cellRenderer: (p: ICellRendererParams<ResearchRow>) => {
            ...
            return (
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                {inProgress && <Chip size="small" label="관리자 검토요청" sx={StatusMap.inprogressReviewDept1.chipStyle} />}
                <span className="ellipsis">{p.value ?? ""}</span>
              </Box>
            );
          },
        },
      ...
      <div>
        <div className="tbl_info">
          ...
          <div className="tbl_controller">
            <Button variant="contained" size="medium" onClick={() => navigate(routes.RESEARCH.CREATE)}>
              과제 등록{" "}
            </Button>
          </div>
        </div>
