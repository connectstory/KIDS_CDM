...
export default function MemberFreeBoardListView() {
  ...
  // AG-Grid 컬럼 정의
  // =========================
  const colDefs = useMemo<ColDef<BoardItem>[]>(
    () =>
      [
        {
          headerName: "번호",
    ...
    [appliedState.viewCount, appliedState.currentPage, totalCount]
  );
  ...
  return (
    <div className="aggridguard-page">
      ...
      {/* 자유게시판 안내 문구 (freeboard만 표시) */}
      {resolvedBoardType === "freeboard" && (
        <div className="mt-4 mb-6 p-4 bg-amber-50 border border-amber-200 rounded text-gray-700">
          ⚠️ 개인정보 유출, 타인 비방, 욕설 등은 법률에 의해 제재될 수 있습니다.
        </div>
      )}
      {resolvedBoardType !== "freeboard" && <div className="h-5" />}
