...
import { ANALYSIS_ORG_ATTACHMENT_ACCEPT, isPreviewableFile } from "@/constants/researchFileUpload";

export default function OrgDataManagementWrite({
  asmtMetaRsltSn,
  onConfirm,
  onCancel,
}: {
  ...
  const handleSubmit = () => {
    ...
    if (displayFiles.length === 0) {
      showAlert({ message: "기관 데이터를 첨부해주세요.", severity: "warning" });
      return;
    }
      ...
      updateAnalysisDataMutation.mutateAsync(
        {
          ...
          data: {
            ...
            rsltGroupCd: analysisDataDetail?.rsltGroupCd || RsltGroupStcdType.ANALYSIS_ORG,
          },
          deleteFileIds: deleteFileIds.length > 0 ? deleteFileIds : undefined,
          files: uploadFiles.length > 0 ? uploadFiles : undefined,
        },
        { onSuccess: () => onConfirm({ isEditMode: true, asmtMetaRsltSn }) }
      );
    } else {
        ...
        <Stack direction="row" className="form_container-row">
          <Box className="form_container-column">
            <Box className="form_container-row-label">
              <Typography className="required">기관 데이터 첨부</Typography>
            </Box>
            <Box className="form_container-row-content w-full">
              <Stack className="w100" direction="column" spacing={CONTENT_GAP.XSMALL}>
                ...
                />
                {/* 파일 업로드 영역 */}
                <FileDropZone onDrop={handleFileDrop} acceptExtensions={ANALYSIS_ORG_ATTACHMENT_ACCEPT} />
                <Typography variant="description">작성하신 기관 데이터를 첨부해주세요.</Typography>
              </Stack>
            </Box>
          </Box>
        </Stack>
