...
import { deleteQna, fetchQnaAnswer, fetchQnaDetail, increaseQnaViewCount } from "@/api/communityApi";
...
export default function QnaDetailView() {
  ...
  const { id, boardType } = useParams<{ id: string; boardType: BoardType }>();
  ...
  useEffect(() => {
    ...
    const viewKey = `QNA_VIEWED_${id}`;
    if (sessionStorage.getItem(viewKey)) return;

    increaseQnaViewCount(id)
      .then(() => {
  ...
  const isAuthor = !!currentMbrId && !!qnaData?.rgtrId && currentMbrId === qnaData.rgtrId;
          ...
          <Box className="form_container">
            <Stack direction="row" className="form_container-row">
              <Box className="form_container-column">
                ...
                <Box className="form_container-row-content">
                  <Typography>{ansData.ansrNm || "-"}</Typography>
                </Box>
              </Box>
              <Box className="form_container-column">
                <Box className="form_container-row-label">
        ...
        {!ansData && (
          <>
            ...
            <Button
              variant="contained"
              color="error"
              disabled={!isAuthor || deleteQnaMutation.isPending}
              onClick={() => {
