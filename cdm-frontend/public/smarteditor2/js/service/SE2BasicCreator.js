...
window.createSEditor2 = function(elIRField, htParams, elSeAppContainer){
	...
	const elAppContainer = (elSeAppContainer || jindo.$("smart_editor2"));	
	const elEditingArea = jindo.$$.getSingle("DIV.husky_seditor_editing_area_container", elAppContainer);
	const oWYSIWYGIFrame = jindo.$$.getSingle("IFRAME.se2_input_wysiwyg", elEditingArea);
	const oIRTextarea = elIRField?elIRField:jindo.$$.getSingle("TEXTAREA.blind", elEditingArea);
	const oHTMLSrc = jindo.$$.getSingle("TEXTAREA.se2_input_htmlsrc", elEditingArea);
	const oTextArea = jindo.$$.getSingle("TEXTAREA.se2_input_text", elEditingArea);
	...
	const oEditor = new nhn.husky.HuskyCore(htParams);
	...
	if(htParams.bSkipXssFilter !== true){
		...
		oEditor.registerPlugin({
			...
			$ON_REGISTER_CONVERTERS : function() {
				const fXssFilter = jindo.$Fn(function(sHtml){
