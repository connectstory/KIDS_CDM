...
export const DISCLOSURE_STATUS_MAP: Record<string, string> = {
  "01": "등록",
  "02": "진행중",
  "03": "마감",
};
...
// 구분 코드 매핑
export const DISCLOSURE_TYPE_MAP: Record<string, string> = {
  "01": "정기",
  "02": "비정기",
};
...
type DisclosureListResponseRaw = DisclosureListResponse & {
  uld_inst_prgrs_stts_cd?: string | null;
  uld_type_cd?: string | null;
};

/**
 * DisclosureListResponse 정규화 (snake_case 키가 오면 camelCase로 합침)
 */
function transformDisclosureListResponse(response: DisclosureListResponse): DisclosureListResponse {
  const r = response as DisclosureListResponseRaw;
  return {
    ...response,
    uldInstPrgrsSttsCd: r.uldInstPrgrsSttsCd ?? r.uld_inst_prgrs_stts_cd ?? undefined,
    uldTypeCd: r.uldTypeCd ?? r.uld_type_cd ?? undefined,
  };
}
...
export const DisclosureAPI = {
   ...
   * @returns 공시 상세 정보
   */
  getDisclosureById: async (id: string) => {
   ...
   */
  updateDisclosure: (id: string | number, payload: DisclosureUpdateRequest) => axios.put<void>(`${BASE_URL}/${id}`, payload),

  /**
   * 공시 삭제
   * @param id 공시일련번호 (pblntSn)
   */
  deleteDisclosure: (id: string | number) => axios.delete<void>(`${BASE_URL}/${id}`),
   ...
   * @returns 파일 목록
   */
  getFilesByPblntSn: async (pblntSn: string, ptcpInstSn?: string | null, fileSeCd?: string | null) => {
    const params: Record<string, unknown> = {};
    if (ptcpInstSn != null) {
      params.ptcpInstSn = ptcpInstSn;
    }
    if (fileSeCd != null && fileSeCd !== "") {
      params.fileSeCd = fileSeCd;
    }
   ...
   */
  deleteFile: async (pblntSn: string | number, atchFileId: string) => {
   ...
   * @returns 참여기관 목록
   */
  getPartnersByPblntSn: async (pblntSn: string) => {
   ...
   */
  getPartnerPickerBusyInstIds: async (pblntSn: string | number) => {
   ...
   */
  addPartners: async (pblntSn: string | number, request: DisclosurePartnerRequest) => {
   ...
   */
  deletePartner: async (pblntSn: string | number, ptcpInstSn: number) => {
   ...
   * @param ptcpInstSn 참여기관번호
   */
  getPartnerStatus: async (pblntSn: string | number, ptcpInstSn: number) => {
   ...
   * @param ptcpInstSn 참여기관번호
   */
  getCancelReason: async (pblntSn: string | number, ptcpInstSn: number) => {
   ...
   */
  updateCancelReason: async (pblntSn: string | number, ptcpInstSn: number, cancelReason: string) => {
   ...
   */
  updateDisclosureStatus: async (pblntSn: string | number, pblntStcd: string) => {
   ...
   */
  closeDisclosure: async (pblntSn: string | number) => {
   ...
   */
  checkAndCompleteDisclosure: async (pblntSn: string | number) => {
   ...
   */
  confirmUploadStats: async (pblntSn: string | number, ptcpInstSn: number) => {
   ...
   * @returns 업로드 통계 정보
   */
  getUploadStats: async (pblntSn: string | number, ptcpInstSn?: number | null) => {
   ...
   */
  getConsistencyHeaderDetail: async (pblntSn: string | number, ptcpInstSn: number, errTblNm: string) => {
    const response = await axios.get<{
      data: {
        data: {
   ...
   */
  getUploadStatsHistory: async (pblntSn: string | number, ptcpInstSn?: number | null) => {
   ...
   * @returns 카탈로그 목록
   */
  getCatalog: async (pblntSn: string | number, ptcpInstSn?: number | null, tblSeCd?: string | null) => {
    const params: any = {};
   ...
   * @param ptcpInstSn 참여기관일련번호
   */
  getPartnerInformation: async (pblntSn: string | number, ptcpInstSn: number) => {
  ...
  getUldPrgrsYn: async (pblntSn: string | number, ptcpInstSn: number) => {
  ...
  clearUldPrgrsYn: async (pblntSn: string | number, ptcpInstSn: number) => {
   ...
   */
  getStatusInfoHistory: async (pblntSn: string | number, ptcpInstSn: number) => {
   ...
   */
  resetUploadData: async (pblntSn: string | number, ptcpInstSn: number) => {
   ...
   * CDM 검증 진행상태 조회
   */
  cdmValidateStatus: async (pblntSn: string | number, taskId: string) => {
    const response = await axios.get<{
      status: string;
      message: string;
      data: {
